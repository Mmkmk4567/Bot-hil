...
error_reporting(0);
#-----------------------------#
$token = "6097269846:AAF9-XLGXlA9p38a7HZa_cacrZICZVcFu5A"; //Token
$dev = "5716701585"; //Id Admin
#-----------------------------#
...
if(!is_dir("data/vpn/v2raytest")){
mkdir("data/vpn/v2raytest");
}
...
[['text'=>"🛍 خرید سرویس"]],
[['text'=>"👤 حساب کاربری"],['text'=>"📱وضعیت سرویس ها"]],
[['text'=>"📚 آموزش اتصال"],['text'=>"➕ افزایش موجودی"]],
],
'input_field_placeholder'=>"Apple Vpn",
'resize_keyboard' =>true]);
$key2 = json_encode(['keyboard'=>[
[['text'=>"🎁 کانفیگ تست"],['text'=>"🛒 کانفیگ فروشی"]],
[['text'=>"$o"]],
],
'input_field_placeholder'=>"Apple Vpn",
'resize_keyboard' =>true]);
...
[['text'=>"افزودن کانفیگ تست"],['text'=>"افزودن کانفیگ"]],
[['text'=>"$oo"]],
],
'input_field_placeholder'=>"Apple Vpn",
'resize_keyboard' =>true]);
...
[['text'=>"♧ تنظیم متن راهنمای اتصال"],['text'=>"➕ افزودن موجودی"]],
...
$tv2ray = count ($scan) - 2;
$scan1 = scandir ("data/vpn/v2raytest");
$tex = count ($scan1) - 2;
...
[['text' =>"$tex",'callback_data'=>"a"],['text'=>"$ex",'callback_data'=>"a"],['text' =>"کانفیگ تست",'callback_data'=>"a"]],
...
#-----------------------------#
if($text == "🎁 کانفیگ تست"){
$ok1 = scandir ("data/vpn/v2raytest");
$ok  = count ($ok1) - 2 ;
$ok2 = file_get_contents ("data/user/$from_id/vpntest");
if($ok2 == "true"){
    bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"شما قبلا اکانت تست دریافت کردید .",
    'reply_markup'=>$back,
    'parse_mode'=>"Markdown",
    ]);
    file_put_contents ("data/user/$from_id/step.txt","none");
...
if($ok < 1){
    ...
    bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"در حال ساخت اکانت تست شما",
    'parse_mode'=>"Markdown",
    ]);
    
    sleep ('4');
$scan = scandir("data/vpn/v2raytest");
$random = $scan[rand(2, count($scan) - 1)];
$a = file_get_contents ("data/vpn/v2raytest/$random");
    bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"
    اکانت تست شما :
    ...
    file_put_contents ("data/user/$from_id/vpntest","true");
    unlink ("data/vpn/v2raytest/$random");
}
}
#-----------------------------#
if($text == "📚 آموزش اتصال"){
...
#-----------------------------#
if($text == "افزودن کانفیگ تست"){
    bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"👈 لطفا کد کانکشن رو برای من ارسال کنید .",
    'reply_markup'=>$bk,
    'parse_mode'=>"Markdown",
    ]);
    file_put_contents ("data/user/$from_id/step.txt","v2raytest");
}
if($step == "v2raytest" and $text != $oo){
$rand = rand (1000,100000);
file_put_contents ("data/vpn/v2raytest/$rand",$text);
    bot('sendmessage',[
    'chat_id'=>$chat_id,
...
#-----------------------------#
if ($text == "➕ افزودن موجودی"){
sendmessage ($chat_id , "مبلغ مورد نظر را به تومان و با اعداد وارد کنید ‌." , $bk);
file_put_contents ("data/user/$from_id/step.txt","pluscoin");
}
if ($step == "pluscoin" and $text != $oo){
file_put_contents ("data/plus","$text");
file_put_contents ("data/user/$from_id/step.txt","hale");
Sendmessage ($chat_id , "ایدی کاربر را ارسال کن" , $bk);
}
if($step == "hale" and $text != $oo){
$b = file_get_contents ("data/plus");
$coink = file_get_contents ("data/user/$text/coin");
$a = $coink + $b;
File_put_contents ("data/user/$text/coin",$a);
Sendmessage ($chat_id , "انجام شد" , $bk);
Sendmessage ($text , "از طرف مدیریت مبلغ $b تومان برای شما ارسال شد .");
file_put_contents ("data/user/$from_id/step.txt","none");
}
#-----------------------------#
} //
